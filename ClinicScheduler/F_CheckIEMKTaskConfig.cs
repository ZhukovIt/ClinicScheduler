...
{
    public partial class F_CheckIEMKTaskConfig : Form
    {
        const string TaskName = "CheckIEMKRecordsState";
        ...
        public F_CheckIEMKTaskConfig(AppConfig config)
        {
            this.config = config;
            InitializeComponent();
        ...
        public F_CheckIEMKTaskConfig(AppConfig config, CClinicApp clinicApp)
        {
        ...
        private void fillControls()
        {
            if (config != null)
            {
                if (config.CheckIEMKRecordsStateProperties != null)
                {
                    this.cb_Medical.SelectedValue = config.CheckIEMKRecordsStateProperties.ClinicId;

                    this.cb_Branch.SelectedValue = config.CheckIEMKRecordsStateProperties.BranchId;
                    if(config.CheckIEMKRecordsStateProperties.UserId != 0)
                        this.cb_User.SelectedValue = config.CheckIEMKRecordsStateProperties.UserId;

                    this.tb_NubmerOfDays.Text = config.CheckIEMKRecordsStateProperties.NumberOfDays.ToString();
                }
            }
        }
        ...
        private void btn_Save_Click(object sender, EventArgs e)
        {
            if (validationControls())
            {
                ...
                if (config.CheckIEMKRecordsStateProperties == null)
                    config.CheckIEMKRecordsStateProperties = new CheckIEMKRecordsStateProperties();

                config.CheckIEMKRecordsStateProperties.BranchId = selectedBranchId;
                config.CheckIEMKRecordsStateProperties.ClinicId = selectedOrganizationId;
                config.CheckIEMKRecordsStateProperties.NumberOfDays = numberOfDays_Value;
                config.CheckIEMKRecordsStateProperties.UserId = selectedUserId;
        ...
        private string GenerateTaskCommand()
        {
            ...
            command += "/ST " + dateTimePicker1.Text + " /TN \"SiMed_Задача_" + TaskName + "\" /TR \"'" +
                Application.ExecutablePath + "' CheckIEMKRecordsState\"";
